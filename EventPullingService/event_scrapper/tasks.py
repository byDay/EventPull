...
def process_venue_scrapping(venue_id):
    ...
    log = models.ScrapingEventLogs.objects.create(
        venue=venue, status=0, start_time=datetime.datetime.today())
    error_message = {}
    ...
    try:
        ...
        total_events = len(event_obj_list)
        uploaded_events = 0
        failed_events = 0
        for event_obj in event_obj_list:
            ...
            if serialized_event_data.is_valid():
                event_obj['venue'] = {
                    "id": venue.venue_metadata.get('atl_venue_id')}
                created = WPDataHelper.create_wp_event(event_obj)
                if type(created) == dict:
                    serialized_event_data.save()
                    uploaded_events = uploaded_events + 1
                else:
                    error_message[event_obj['event_url']] = created
                    failed_events = failed_events + 1
                status = 1
            else:
                status = 2
                error_message[event_obj['event_url']
                              ] = serialized_event_data.errors
    ...
    log.status = status
    log.description = {'error_message': error_message, 'total': total_events,
                       'uploaded': uploaded_events, 'failed': failed_events}
    log.save()


# Daily Cron
@celery.decorators.periodic_task(run_every=crontab(minute=0, hour=8))
def pull_data_from_atlbyday_wordpress():

    event_pull_log_obj = models.AtlPullEventLogs.objects.create(
        status=0, start_time=datetime.datetime.today())
    ...
    try:
        ...
        for category in all_category:
            category_message = atl_data_helper_obj.update_or_create_category(
                category)
            if category_message:
        ...
        for organizer in all_orgainzer:
            organizer_message = atl_data_helper_obj.update_or_create_organizer(
                organizer)
        ...
        pull_data_result = {"event": event_messages, "organizer": organizer_messages,
                            "venue": venue_messages, "category_messages": category_messages, "tag_messages": tag_messages}
